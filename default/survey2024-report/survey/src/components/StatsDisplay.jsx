function StatsDisplay({ result }) {
  ...
  if (result.type === 'single_choice' || result.type === 'multiple_choice') {
    ...
    const total = isSingleChoice ? result.total_responses : result.total_respondents
    const displayLabel = isSingleChoice ? '回答数:' : '回答者数:'

    const topItems = Object.entries(result.counts)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3)
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        ...
        <div className="space-y-3 text-sm">
          {rows.map(([rowLabel, rowData]) => (
            <div key={rowLabel} className="pb-3 border-b border-gray-200 last:border-0">
              <div className="font-medium text-gray-700 mb-1">{rowLabel}</div>
              <div className="pl-2 space-y-1">
                <div className="flex justify-between">
                  <span className="text-gray-600">平均:</span>
                  <span className="font-medium">{formatNumber(rowData.average || 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">回答数:</span>
                  <span className="font-medium">{rowData.total_responses || 0}</span>
                </div>
              </div>
            </div>
          ))}

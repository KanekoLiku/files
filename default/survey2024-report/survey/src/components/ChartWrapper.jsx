...
function ChartWrapper({ qdata, result }) {
	...
	if (result.type === "multiple_choice") {
		let counts = result.counts;
		let total = result.total_respondents || 0;
		...
		if (counts && typeof counts === "object") {
			...
			if (keys.length === 1 && counts[keys[0]] && counts[keys[0]].counts) {
				...
				total =
					nestedData.total_respondents || nestedData.total_responses || total;
			}
		}
						...
						{rows.map(([rowLabel, rowData]) => (
							<div key={rowLabel} className="bg-gray-50 rounded p-3">
								...
								<div className="text-sm text-gray-600">
									{Object.entries(rowData.counts || {}).map(
										([option, count]) => {
											const percentage =
												rowData.total_responses > 0
